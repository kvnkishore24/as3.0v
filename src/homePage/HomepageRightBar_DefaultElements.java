 ...
 */
public class HomepageRightBar_DefaultElements extends ReusableMethods {
	Logger log = Logger.getLogger(HomepageRightBar_DefaultElements.class.getName());
	...
	@Test(groups = { "SuccessfulLogin" }, dependsOnGroups = "BETA_LOGIN",dependsOnMethods = "checkLoginFormElementsPresence", description = "Testing login Functionality.")
	public void indexesPresence() {
		log = Logger.getLogger(Thread.currentThread().getStackTrace()[1].getMethodName());
		PropertyConfigurator.configure("Log4j.properties");
		log.info("TESTCASE::______________" + Thread.currentThread().getStackTrace()[1].getMethodName());
		try {
